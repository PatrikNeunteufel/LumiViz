...
Die Grundlagen stehen im Vorgänger und werden hier nicht wiederholt: **Die drei Import-Wege** (Copy & Paste in den Shadertoy-Node, URL-/ID-Import mit App-Key, Shadertoy-Browser-Panel) **und die allgemeine Portabilitäts-Checkliste findest du in „Crystal-Lights-Shader-Tutorial.md", Anhang B.** Unser Shader hält sich an dieselbe Konvention wie die 100 Vorrats-Shader in `asset/shadertoys/` (nur Standard-Uniforms, STELLSCHRAUBEN-Block) – alle drei Wege funktionieren also unverändert. Hier nur das, was an *diesem* Shader besonders ist: Er ist **Multipass mit Selbstreferenz**. *(UI-Namen und Verhalten: Stand Session 65/67.)*
