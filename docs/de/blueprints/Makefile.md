...
- **Robustheit** — Fail-Fast bei Konfigurationsfehlern
...
└── make/
    ...
    │   ├── toolchain.mk       /* Compiler-Konfiguration         */
...
| Kategorie | Verantwortlichkeit | Beispiele |
|-----------|-------------------|-----------|
| **Bootstrap** | Basis-Funktionen, Argument-Prüfung | `bootstrap.mk`, `arguments.mk` |
...
```makefile
# <module_name>.mk - <kurze Beschreibung>
# <optionale Details>
...
# ❌ Schlecht: Keine Fehlerprüfung
$(CC) $(CFLAGS) -o $@ $<

# ✅ Gut: Mit Fehlerprüfung
@"$(CC)" $(CFLAGS) -o "$@" "$<"
@[ -f "$@" ] || (echo "[FATAL] Compilation failed: $<"; exit 2)
```
