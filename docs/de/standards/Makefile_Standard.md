...
> **Geltungsbereich:** Alle Makefile-basierten Build-Systeme  
...
Build-System-Entwickler, die Makefiles für Embedded- oder PC-Projekte schreiben.
...
2. **Fail-Fast** — Fehler früh erkennen und klar melden
...
| Aspekt | Regel |
|--------|-------|
| Einrückung in Rezepten | **Tab** (Pflicht in Make) |
...
### 9.2 Fehler in Rezepten

```makefile
# Fehlerprüfung nach Befehl
$(OUT)/app.out: $(OBJS)
	@"$(LD)" -o "$@" $(OBJS)
	@[ -f "$@" ] || (echo "[FATAL] Link failed: $@"; exit 2)
```

### 9.3 Fehlermeldungs-Format

```
[FATAL] <Beschreibung> - <Kontext>
[WARN]  <Beschreibung>
[INFO]  <Beschreibung>
```
...
- [Makefile_Build_System_Concept.md](Makefile_Build_System_Concept.md) — Referenz-Implementierung
