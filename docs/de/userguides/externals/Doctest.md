...
> **Sprache:** Deutsch  
> **English:** [doctest.md](../../../en/userguides/externals/Doctest.md)
...
| Assertion | Beschreibung | Bei Fehler |
|-----------|--------------|------------|
...
TEST_CASE("Parameterized test") {
    ...
    for (const auto& [input, expected] : test_data) {
        CAPTURE(input);  // Zeigt Wert bei Fehler
...
TEST_CASE("User validation") {
    ...
    // Schlecht: Keine Info bei Fehler
    CHECK(user.isValid());
...
- [Local_Externals_Testing.md](../../references/externals/Local_Externals_Testing.md) — Referenz
