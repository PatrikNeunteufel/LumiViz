...
| 1.5.0 | 2026-08-02 | Session 65 (Strang R, Regelwerk): **`TranspileOptions`** mit `d3d9Div` (Default true = 1.4.0-Verhalten, alle Bestands-Aufrufer unverändert). `false` ⇒ roher `/`-Operator für das Modern-Regelwerk (IEEE, keine Legacy-Krücken); `transpile()` nimmt die Optionen als drittes Argument mit Default |
