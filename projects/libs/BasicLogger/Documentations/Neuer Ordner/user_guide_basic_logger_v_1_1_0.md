...
## 10. Eintrag in Solution.json

Damit der `BasicLogger` im Buildsystem verfügbar ist, muss er in der `Solution.json`
unter `libraries` eingetragen sein.
...
- `name` → Identifikation im Dependency-System
- `version` → Version muss zur Modulversion passen
- `type` → `INTERFACE` bedeutet header-only
- `public_headers` → Pfad zum Include-Verzeichnis

**Verwendung:**
Executables müssen `"dependencies": ["BasicLogger"]` angegeben haben.
