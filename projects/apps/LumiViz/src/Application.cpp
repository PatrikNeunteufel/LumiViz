...
#include <QSettings>  // Uebernahme der MyViz-Einstellungen (S73)
#include <QDir>
...
/**
 * @brief Uebernimmt die Einstellungen aus der MyViz-Zeit (S73, einmalig).
 *
 * Vor der Umbenennung lagen sie unter Organisation „MyViz Project" /
 * Anwendung „MyViz"; jetzt unter „LumiViz Project" / „LumiViz". Ohne
 * Uebernahme waere alles scheinbar verloren: Fensteranordnung, Hotkeys,
 * GPU-Wahl, zuletzt geladenes Preset.
 *
 * Kopiert wird NUR, wenn am neuen Ort noch nichts steht — ein zweiter Lauf
 * darf spaetere Aenderungen nicht mit alten Werten ueberschreiben. Die alten
 * Werte bleiben liegen (kein Loeschen): faellt jemand auf eine aeltere
 * Fassung zurueck, findet er sie unveraendert vor.
 */
void migriereAlteEinstellungen()
{
    QSettings neu;
    if (!neu.allKeys().isEmpty()) return;  // schon eingerichtet — nichts tun

    QSettings alt(QStringLiteral("MyViz Project"), QStringLiteral("MyViz"));
    const QStringList schluessel = alt.allKeys();
    if (schluessel.isEmpty()) return;  // Neuinstallation — nichts zu holen

    for (const QString& k : schluessel)
    {
        neu.setValue(k, alt.value(k));
    }
    neu.sync();

    BasicLogger::logInfo("Einstellungen aus der MyViz-Zeit uebernommen: " +
                         std::to_string(schluessel.size()) + " Schluessel");
}
...
struct Application::Impl
{
    ...
    // -------------------------------------------------------------------------
    // Name und Version kommen aus Solution.json (S73) — siehe AppInfo.hpp.
    // NICHT wieder hart eintragen: genau hier stand vorher eine zweite,
    // nie nachgezogene Version.
    std::string name{lumi::appName().toStdString()};
    std::string version{lumi::appVersion().toStdString()};
    ...
    : m_impl{std::make_unique<Impl>()}
{
    ...
    // -------------------------------------------------------------------------
    BasicLogger::setLogFile((lumi::appName() + QStringLiteral(".log")).toStdString());
...
bool Application::init(int argc, char* argv[])
{
    ...
    m_impl->pQtApp->setOrganizationName(QStringLiteral("LumiViz Project"));

    // Einstellungen aus der MyViz-Zeit uebernehmen (S73, einmalig).
    //
    // Organisation und Anwendungsname bilden den QSettings-Pfad. Mit der
    // Umbenennung MyViz -> LumiViz zeigt er woandershin, und ohne diesen
    // Schritt stuende jeder bestehende Nutzer vor einer nackten App:
    // Fensteranordnung, Hotkeys, GPU-Wahl, zuletzt geladenes Preset — alles
    // scheinbar weg, obwohl es nur nebenan liegt.
    migriereAlteEinstellungen();
