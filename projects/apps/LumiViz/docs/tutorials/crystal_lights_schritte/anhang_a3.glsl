// ============================================================================
// Anhang A3 - Das Kristallfeld hoert zu
// Gesamtlisting aus Schritt 14 + die Einbau-Diffs (a)/(b)/(c) aus Anhang A3
// (Mappings 1-6 aus A2). Rekonstruiert aus CrystalLights-tutorial.md (SSOT).
// iChannel0: Music
// ============================================================================
...
// GEAENDERT [5]: Schwelle atmet mit der Lautheit
float liquid(vec2 p)
{
...
// GEAENDERT [6]: Energie reisst die Decke auf
...
// GEAENDERT [1]: das Beat-Gate zuendet alle Lampen gemeinsam
float blink(vec2 id)
{
...
// GEAENDERT [3]: Farbwelt wandert mit den Mitten
vec3 lightsAt(vec2 q, float weich)
{
    ...
    for (int x = -1; x <= 1; x++) {
        ...
        acc += lampColor(hash21(id) + gMid * 0.4) * hell / (0.02 + dot(d, d) * 14.0 / weich);
    }
    return acc * 0.05;
}
...
// GEAENDERT [2][4]: Glow pumpt mit dem Bass, Sparkle mit den Hoehen
vec3 shadeKristall(vec3 p, vec3 rd, float t)
{
    ...
    col += max(dot(n, mond), 0.0) * vec3(0.10, 0.14, 0.20);
...
{
    // (b) Audio-Globals einmal pro Frame fuellen, BEVOR Marsch und Kamera laufen
