...
- **`StratosphericTunnel-tutorial.md`** – die **Wirtsszene**: Röhrenwand mit Neonfugen, Fenstermaske auf der abgerollten Wand-Karte `(w, z)`, Kamera-Scheinwerfer, Vortrieb entlang der Achse. Von hier stammen `tunnelMap`, das Fensterraster und die Neon-Emission.
- **`SpaceDebris-tutorial.md`** – die **Außenwelt hinter den Fenstern**: 3D-Zellgitter mit Zellregel und Zellwand-Klammer, taumelnde Brocken als Funktion der Zell-Id, der analytische Glutplanet mit Atmosphären-Saum, die Kamera-Blase. Von hier stammen `debrisMap`, `planetHit` und das ganze Taumel-Besteck.
- **`CrystalLights-tutorial.md`** – der **Boden-Merge**: Höhenfeld mit Voronoi-Platten, der Lücken-Trick (`mix(-6, h, maske)`), die vereinfachte Brechung mit Beer-Lambert-Absorption und die `1/d²`-Lampen unter dem Kristall. Von hier stammen `kristallHoehe`, `kristallLampen` und `shadeKristall`.
...
- LumiViz/LumiViz mit Shadertoy-Node in der Effect-Chain (Stand Session 65/67).
...
![Schritt 1: das Tunnel-Skelett – Röhrenwand, Neonfugen, dunkle Fensteröffnungen](composite_portals_bilder/schritt_01.png)
...
![Schritt 2: das Debris-Skelett – taumelnde Brocken über dem Glutplaneten](composite_portals_bilder/schritt_02.png)
...
![Schritt 3: Split-Screen als Kollisions-Beweis – links der Tunnel, rechts das Trümmerfeld, eine Datei](composite_portals_bilder/schritt_03.png)
...
![Schritt 4: das Portal – durch die Fensteröffnungen die echte Debris-Welt](composite_portals_bilder/schritt_04.png)
...
![Schritt 5: Maßstab 4 – die Fenster zeigen die Diorama-Außenwelt mit ganzen Trümmerschwärmen](composite_portals_bilder/schritt_05.png)
...
![Schritt 6: atmende Portale mit pulsierendem Neon-Rahmen](composite_portals_bilder/schritt_06.png)
...
![Schritt 7: Material-Id – die Debug-türkise Bodenplatte schiebt sich in die Röhre](composite_portals_bilder/schritt_07.png)
...
![Schritt 8: das Kristall-Höhenfeld – zerklüftete Schollenkante, noch in Debug-Türkis](composite_portals_bilder/schritt_08.png)
...
![Schritt 9: Kristall-Shading – Lampen glühen unter dem Boden, das Neon spiegelt sich](composite_portals_bilder/schritt_09.png)
...
![Schritt 10: pixelgenaue Portal-Kanten per fwidth (AA-Stellschraube auf dem Default 1)](composite_portals_bilder/schritt_10.png)
...
// GEAENDERT: tunnelShade - MOTIVKOPPLUNG: das Licht aus dem gegenueberliegenden
...
![Schritt 11: Kohärenz – eine Uhr, eine Palette, ein Tonemapping; die rollende Kamera über der Kristallzone](composite_portals_bilder/schritt_11.png)
...
![Schritt 12: das fertige Composite – drei Welten, ein Strahl, eine Klammer](composite_portals_bilder/schritt_12.png)
...
![Anhang A1: das Beat-Gate (synthetisches Testsignal des Standalone – dessen Bass liegt auf der Shadertoy-dB-Skala dauerhaft über der Gate-Schwelle, darum stehen beide Seiten voll)](composite_portals_bilder/anhang_a1.png)
...
![Anhang A3: das Composite hört zu – beat-geöffnete Portale, gepumpte Glut (synthetisches Testsignal)](composite_portals_bilder/anhang_a3.png)
...
**Ehrlichkeits-Hinweis:** Wie die ganze Serie ist auch dieses Tutorial am Schreibtisch konstruiert und grob gegengerechnet (Zellregel, Boden-Sehne, Schranken-Konservativität, Budget-Überschläge) und inzwischen **in LumiViz gegengerendert** (alle 14 Schritte kompilieren warnungsfrei auf Anhieb; die Screenshots im Text stammen aus diesen Läufen) – auf shadertoy.com ist der Sichttest weiterhin offen. Fürs Feintuning bleiben die üblichen Verdächtigen: die Helligkeits-Balancen zwischen den drei Welten (`T_NEON`, `D_GLUT`, der Lampen-Faktor `0.06`, `BELICHTUNG`), die Blasen- und Schranken-Konstanten und die `fwidth`-Untergrenze. Die Architektur trägt; die Zahlen sind Startwerte.
