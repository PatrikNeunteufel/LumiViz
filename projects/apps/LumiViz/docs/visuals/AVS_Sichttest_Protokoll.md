...
| S15 | **„Alien Alloy" schwarz bis auf die Sprites** (Rest der S51-Arbeit, 0,647) | Texer II belegte `red`/`green`/`blue` **je Punkt** neutral vor — also NACH dem Frame-Slot, in dem das Preset sie berechnet (`red=sin(ct+2.07)*0.5+0.5` …). Alle Sprites blieben weiss; da sie die einzige Energiequelle des Wirbels sind, lief das Bild ueber die Frames nach Schwarz (Frame 1 dMean 0,010 -> Frame 120 0,568). Bisektion: `l05` 0,000, `l06` (+1 Texer) kippt, ohne die Dynamic Movement wieder gut -> Minimalfall aus zwei Knoten. Paar-Sonde `6_alloy/texer_farbe_frame` vs `_point`: dieselbe Farbe im Point-Slot referenzgleich (MAE 0,001), im Frame-Slot abweichend (0,042) — die Referenz laesst den Frame-Wert die Punktschleife ueberleben | ✅ **gefixt (S52):** Farb-Vorbelegung einmal je Frame VOR dem Frame-Slot, wie `sizex`/`sizey` seit S51. Alien Alloy 0,647 -> **0,008**; Sonden 63/64, Matrix 37/41 unveraendert, Zwillinge 65/65 | ✅ |
| S16 | **Mirror spiegelt nicht sauber** (Befund Patrik: greatwho2006 15/16 „sollten sauber spiegeln") | Unser Shader wertete alle VIER Richtungsregeln in EINEM Durchgang aus und las dabei stets die UNVERAENDERTE Textur — die Zuweisungen ueberschrieben sich gegenseitig. Das Original laeuft vier Schleifen NACHEINANDER ueber den Framebuffer (`r_mirror.cpp` 167/188/210/230: VERTICAL1, VERTICAL2, HORIZONTAL1, HORIZONTAL2), jede sieht das Ergebnis der vorigen — genau darauf beruht, dass zwei aktive Achsen ein symmetrisches Bild ergeben. Messung der Spiegel-Symmetrie beider Ausgaben: Referenz **0,0000** in beiden Achsen, wir 0,042 (Preset 15) bzw. 0,040 (16) | ✅ **gefixt (S52):** ein Durchgang je aktiver Richtung in der Reihenfolge der Referenz; Smooth-Rampe unveraendert. Symmetrie jetzt **0,0000** wie die Referenz; Matrix `26_mirror` und Zwillinge unveraendert | ✅ |
| S17 | **Beat-getriebene Presets sind mit `--beat-period 0` gar nicht vergleichbar** | Beide Renderer erkennen Beats SELBST aus dem synthetischen Audio und kommen dabei auf verschiedene Zeitpunkte. Wo ein Effekt je Beat Zustand aufbaut, divergiert er dadurch zwangslaeufig — ohne dass ein Render-Fehler vorliegt. Beleg Moving Particle (greatwho2006 16, Stufe l02): dessen Physik ist zeilengleich mit `r_parts.cpp` (Feder 0.004, Daempfung 0.991, `ss=min(h/2,w*3/8)`, `maxdist/32`), je Beat aber wird ein neues Ziel `c` gewuerfelt und damit Energie eingespeist. Gemessen: beat-period 0 -> MAE 0,042 · 24 -> 0,005 · **60 -> 0,001** | Methodik: beat-getriebene Presets mit festem `--beat-period` vergleichen. Offen bleibt die Frage, ob unsere Beat-RATE der von Winamp entspricht (steht seit S44 in §4) | ✅ Methodik |
| S18 | **Custom BPM (id 33) laesst eine andere Zahl Beats durch** | Sonde `7_rand/bpm_zaehler_skip3`: ein Scope, dessen Beat-Slot nur zaehlt, als Zeilenlage. Ohne Filter referenzgleich (MAE 0,000), mit `skip=1, skipval=3` liegt die Zeile **9 Positionen** daneben. Trifft Presets, die je Beat Zustand aufbauen — in „Inhaler“ faerbt `hx=hx+rand(100)/50` im Beat-Slot den Ring (unser Ring cyan, Referenz weiss-grau) | ✅ **gefixt (S52):** vier Abweichungen von `r_bpm.cpp:137-185` — (1) **Off-by-one**: durchgelassen wird jeder `skipVal+1`-te Beat, wir nahmen `skipVal` · (2) die drei Betriebsarten sind dort EXKLUSIV (jeder Zweig kehrt sofort zurueck), bei uns liefen sie hintereinander · (3) `skipfirst` wurde geparst, aber nie uebernommen (Feld + Serializer ergaenzt; der Skip-Zaehler laeuft waehrend der Sperre NICHT mit) · (4) `skipval=0` heisst „jeden Beat“, unsere Untergrenze 1 machte „jeden zweiten“ daraus. Sonde jetzt MAE 0,000; Inhaler 0,345 → 0,168 · Reflectosphere 0,174 → 0,085 · Lion King grün | ✅ |
| S19 | **Einzelne Presets sind zwischen Laeufen BISTABIL** | „Inside Gamecube“ liefert auf DEMSELBEN Binary mal dMean 0,045, mal 0,248 — viermal hintereinander gemessen, ohne Codeaenderung; auf dem Vorstand ebenso. Das Preset enthaelt weder Texer noch Mirror noch Custom BPM, kann von den S52-Fixes also gar nicht betroffen sein. Zwei Sweeps gegeneinander zu rechnen haette hier einen Fix beschuldigt, der es nie beruehrt hat | Methodik: **Einzelzahlen mehrfach messen, bevor man sie deutet.** Ursachen-Verdacht: Anlauf-Abhaengigkeit der Rueckkopplung (die ersten Frames entscheiden ueber den Attraktor) — offen | ⬜ |
| S20 | **„Deep Red Sea“ saettigt zu blassem Gelb** (dMean 0,943 — groesster offener Einzelwert) | ZWEI Fehler, per Bisektion getrennt. **(a) Adjustable-Blend vertauscht:** `BLEND_ADJ(*fb, color, v)` = `blendtable[fb][v] + blendtable[color][255-v]` (r_defs.h:250-257, Tabelle i*j/255) — **v gewichtet den FRAMEBUFFER**, wir gaben es der neuen Farbe. Bei v=181 nahmen wir 71 % Farbe statt 71 % Bild. **(b) Clear-Screen-Modus roh in die falsche Tabelle:** `ClearParams::blend` ist eine EIGENE Aufzaehlung (0 replace, 1 additiv, 2 50/50, 3 Line-Blend), `applyLineBlend` erwartet BLEND_LINE (dort ist 2 = MAX, 50/50 = 3). Aus „50/50 gegen Schwarz“ wurde „MAX gegen Schwarz“ = **No-op** — das Bild klang nie ab, 50 Scopes sammelten sich an | ✅ **gefixt (S52):** GL-Faktoren getauscht (`ONE_MINUS_CONSTANT_ALPHA`/`CONSTANT_ALPHA`) + Modus-Abbildung in `runClear`. Deep Red Sea 0,943 → **0,011**; Wtf I'm Lost 0,094 → 0,003 · High Voltage 0,114 → 0,005. Neue Sonden `8_blend/adjustable_{032,181,224}` — der alte Waechter `s9_blend/08` benutzte v=128, wo die Vertauschung symmetrisch und damit UNSICHTBAR ist | ✅ |
| S21 | **„Alternate Reality“ wird durch den korrekten Clear SCHLECHTER** (Vorstand ~0,28 → ~0,68, je dreimal gemessen) | Das Preset klart 50/50 gegen ein helles Orange. Solange unser Clear ein No-op war (S20b), blieb der Hintergrund zufaellig naeher an der Referenz. Mit korrektem Clear saettigt unser Hintergrund pink, die Referenz bleibt WEISS — bei identischer Labyrinth-Struktur. Ein 50/50-Clear gegen Orange muesste auch dort einfaerben; tut es nicht | ⏳ offen — der kaputte Clear hat hier einen ZWEITEN Fehler verdeckt. Naechster Schritt: Bisektion von AR, Augenmerk auf die Listen-Blends um den Clear | ⬜ |
| S22 | **Die BISTABILITAET liegt bei AvsRef, nicht bei uns** (Frage Patrik) | Vier Laeufe desselben Presets, jeder Renderer mit SICH SELBST verglichen: **AvsStandalone MAE 0,0000** (bit-identisch), **AvsRef MAE 0,055-0,064**. Die Referenz ist also nicht reproduzierbar — jeder Vergleich hat eine Rauschgrenze, unter die wir gar nicht messen koennen. `--beat-period` hilft NICHT (0,071), der Beat ist nicht die Quelle. Zwei belegte Mechanismen: **(a)** `r_chanshift.cpp:340` ruft `srand((unsigned int)time(0))` in **`load_config`** — jedes Preset mit einem Channel Shift saet den globalen CRT-Zufallsstrom beim LADEN mit der aktuellen Sekunde neu (im Whacko-Pack: Alternate Reality, High Voltage). **(b)** AvsRef laedt mit `--ape-dir` echte APE-DLLs; deren Code ist Fremdcode und kann eigene Zeitbezuege haben (Milky Way Xtreme hat keinen Channel Shift, streut aber) | Vorschlag: in AvsRef **nach** dem Laden `srand(<fest>)` setzen — neutralisiert (a) und alles, was sich beim Laden aus demselben CRT saet. Bis dahin: Werte solcher Presets **mehrfach messen** (S19) | ⬜ |
| S23 | **Zwei unbekannte APEs** (Befund Patrik, Pack „Whacko AVS IV“) | `Metaballs 3D` und `Tentacles 3D` (beide UnConeD) stehen weder im Parser noch in der Abdeckungs-Matrix — Passthrough. Wirkung: „Yummy Plastics“ und „Rubber Starfish“ bleiben LEER, ihr ganzer Inhalt steckt in dem APE. Inventur des Packs: sonst nur Bekanntes (Multiplier 4×, Color Reduction 2×, Channel Shift 2×) | ⏳ offen — closed-source; entweder Verhaltens-Nachbau wie FyrewurX (S38) oder als bekannte Grenze abhaken | ⬜ |
...
**Setup:** LumiViz-App (Ninja-Clang-Release) und Winamp/AVS nebeneinander,
