...
## ⚠️ WICHTIG: Linker-Workaround

Bei statischen Libraries entfernt der Linker unreferenzierte Translation Units ("dead code elimination"). Da Self-Registration nur interne statische Objekte erzeugt, werden diese vom Linker als "unused" betrachtet.

**Lösung:** `initMenuRegistrations()` muss VOR `buildMenuBar()` aufgerufen werden!
...
{
    // WICHTIG: Verhindert Linker-Optimierung
    initMenuRegistrations();
    
    // Jetzt ist das Registry gefüllt
...
| Datei | Änderung |
|-------|----------|
| **MenuAutoReg.cpp** | Erweitert um Settings-Container, Groups, +initMenuAutoReg() |
| **MenuItemsAutoReg.cpp** | **NEU** - Alle Menu-Items, +initMenuItemsAutoReg() |
| **MenuInit.hpp** | **NEU** - Header für Init-Funktionen (Linker-Workaround) |
| **MenuManager.cpp** | +QActionGroup für exclusive Container |
...
| **MainWindow.cpp** | setupMenuBar() → MenuManager, +initMenuRegistrations(), +setupEventHandlers() |
| **UIEvents.hpp** | +CreateVisualizerEvent, +ResetLayoutEvent |
| **managers/Source.cmake** | +MenuItemsAutoReg.cpp |
| **include/managers/Source.cmake** | +MenuInit.hpp |
...
│  ┌──────────────────────────────┐                                        │
│  │ initMenuRegistrations()      │ ◄── WICHTIG: Verhindert Linker-       │
│  │   ├─ initMenuAutoReg()       │     Optimierung!                       │
