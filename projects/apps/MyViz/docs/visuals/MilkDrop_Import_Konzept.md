...
| 1.11.0 | 2026-07-22 | Session 41: **C1/C2-Sichttest-Befund gelöst** — Regression (tryTranspile-Aufrufe beim C2-Umbau verloren → stiller MD1-Fallback); Fix + Ladepfad-Regressions-Gate + Diagnose-Trace (MilkdropTrace.hpp) + Standalone-Testprogramm MilkdropStandalone (Beweislauf 8/8). **N1+N2 umgesetzt (E1/E2):** MilkdropNodeParams im EffectParams-Variant (Preset eingebettet, Revision-Vertrag), runMilkdropNode (Kern als Node-Engine, Composite-Ziel = gebundenes FBO), ChainSerializer-Key "milkdrop", MultiEffect-Panel-Baum (Sektions-Kinder Code/Waves/Shapes/Shader + Editor), Import-/Save-Routing auf MultiEffect+Node, Standalone-Registry-Eintrag entfernt (Klasse bleibt Engine). Sichttest-Runde offen |
| 1.12.0 | 2026-07-22 | Session 41: **Sprites umgesetzt** (E7-Pflichtkür) — MilkDrop2077-`[SPRITEn]`-Sektionen (SpriteName/ColorKey/Layer/Blend/…-Keys = Startwerte, code_N = per-Frame-EEL) auf das klassische texmgr-Modell gemappt; DrawUserSprites-Port (milkdropfs.cpp:3432-3777): private EEL-VM je Sprite, Referenz-Vertex-Mathe (y-abwärts + finale GL-Negation), Blend 0-4 (alpha/decal/additiv/srccolor/colorkey), Colorkey→Alpha beim Bild-Laden, burn-in in den Feedback-Buffer, done-Kill; Sprite-Persistenz im MilkdropSerializer; Kalibrier-Satz s1. PORT-Annahmen (2077 ohne Quelle): SpriteSpeed skaliert sprite-lokale Zeit, SpriteLayer = Zeichenreihenfolge, 4:3-Burn-Aspekt entfällt |
