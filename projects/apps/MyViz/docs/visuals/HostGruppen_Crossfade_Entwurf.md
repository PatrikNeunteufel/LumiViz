...
| 1.4.0 | 2026-07-23 | **HG2 umgesetzt** (Session 42): Blend-Gewichte folgen `enabled` (Wechsel = Augen-Toggle; Playlist automatisiert das in E6); Doppel-Rendering während des Blends; **Richtungsumkehr statt Snap** (löst den §8-Kleinpunkt); Mix linear per Adjustable (exakter 2er-Mix = HG3-Feinschliff); Buffer-Frischstart nach vollem Ausblenden; Kurven-Hook (linear) |
| 1.5.0 | 2026-07-23 | Sichttest-Befund → **Mix exakt normalisiert** (der sequentielle 2er-Mix aus 1.4.0 benachteiligte die frühere Gruppe quadratisch); **Kurven-Satz entschieden** (§8-Kleinpunkt): Linear · S-Kurve · Ease-In · Ease-Out · Exponentiell, je Gruppe individuell für in/out; **manueller Wechsel-Trigger** im Gruppen-Editor („Zu dieser Gruppe wechseln") als E6-Vorstufe |
| 1.6.0 | 2026-07-23 | **HG3 umgesetzt** (Session 42): Freeze-Frame als automatischer Performance-Fallback (< ~30 fps friert ausblendende Gruppen ein, §4) + progress-Anbindung an die Gruppen-Laufzeit (setProgressOverride; Playlist liefert später die Slot-Dauer). **HG1–HG3 komplett** — nächster Schritt laut §8: Visual-Playlist (E6) |
