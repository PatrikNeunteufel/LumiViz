...
> **Stand:** 2026-07-22 (Session 42) · **Zweck:** EIN Ort für „was ist fertig,
...
|---|---|---|---|
| 0 | **Sichttest-Runde N1/N2 + c1 + m5 (In-App):** c1-/m5-Presets über den neuen Node-Pfad (Import-Browser → MultiEffect+Node) sichten; Panel-Baum + Editor-Sektionen prüfen; Standalone-Werkzeug: `MilkdropStandalone --auto [ordner]` (Kern-Pfad, ohne Chain). *(s1/Sprites: BESTANDEN in S41)* | N1/N2, C1/C2 | Kalibrier-READMEs in `asset/calibration/milkdrop/{c1,m5}/`; Trace-Log `%TEMP%/lumiviz_milkdrop_trace.log` läuft mit |
| 3b | **N3 Panel-Vollausbau:** N3.1 Add/Remove/Clone für Custom Waves/Shapes über das Dropdown-„+"-Muster des Ketten-Editors (Entscheid Patrik S42) · N3.2 numerische Editoren (Preset-Basisparameter decay/zoom/rot/warp/wave_\*/mv_\*/blur\*/Borders/Composite + Wave-/Shape-Init-Params) · N3.3 Sprite-Sektion (heute „parse-only", nur Zähler) | N3 | Datenmodell (16er-Superset) + Revision-Vertrag stehen; Einordnung vor/nach E5: Entscheid Patrik offen |
| 4 | **Crossfade** = echtes Doppel-Rendering, Freeze-Frame nur als Performance-Fallback; Ausbaustufe per-Vertex-Warp-UV-Blend. **Neu (S42): auf Host-Gruppen-Ebene generalisiert** — Entwurf [HostGruppen_Crossfade_Entwurf.md](HostGruppen_Crossfade_Entwurf.md) (Schnitte HG1–HG3, zur Freigabe) | M6 / E5 | zwei Laufzeitzustände (2 Contexts/Feedback/Blur); Entscheide S42: neuer Node-Typ, `.lvfx2`, Settings-Sync je Host-Modul, Tiefenregel 1 Ebene |
