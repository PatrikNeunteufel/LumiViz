...
Übernommen aus `harvest/old_docs/userguide/OpenGL_Context_Handling_UserGuide.md` und
gegen den Code verifiziert (Stand 2026-07-18): Das Pattern ist in **allen 5 Visualizern**
...
Dieses Handbuch beschreibt das **Context-Tracking-Pattern** für OpenGL-basierte
Visualizer in Qt-ADS-Umgebungen (Advanced Docking System).
...
Bei Verwendung von Qt-ADS mit QOpenGLWidget-Visualizern tritt ein kritischer Crash auf,
wenn Dock-Widgets abgedockt werden. Die Ursache: OpenGL-Ressourcen sind an einen
spezifischen Kontext gebunden und werden beim Erstellen eines Floating-Widgets ungültig.
...
- Visualizer erbt von `VisualizerBase`
  ([../../include/visualizers/VisualizerBase.hpp](../../include/visualizers/VisualizerBase.hpp))
- Rendering läuft in einem `QOpenGLWidget` (zentrales VisualizerWidget)
- Implementierung der Override-Punkte `onInitialize()`, `onRender(float)`,
  `onResize(const QSize&)`, `onCleanup()` (die öffentlichen Lifecycle-Methoden
  `initialize/render/resize/cleanup` sind in VisualizerBase `final`)
...
| `ctx->functions()` prüfen | Sicherstellen, dass GL-Aufrufe möglich sind |
...
(entspricht 1:1 der Referenz-Implementierung in
`src/visualizers/PulsingVisualizer.cpp`)
...
Bei Funktionen, die den Viewport lesen, einen Fallback verwenden:
...
**Lösung:** `class QOpenGLContext;` im Header ergänzen.

### 9.2 Kontext nicht gespeichert

**Problem:** Endlosschleife bei Re-Initialisierung.

**Ursache:** `m_lastContext = ctx;` fehlt am Ende von `onInitialize()`.
...
**Lösung:** Vor dem `onInitialize()`-Aufruf:
...
**Lösung:** Jeden OpenGL-Aufruf absichern, auch in Hilfsfunktionen (§8).
...
- [Visualizer_Architecture.md](Visualizer_Architecture.md) — Architektur-Überblick
  (VisualizerBase, Lifecycle)
- [../../include/visualizers/VisualizerBase.hpp](../../include/visualizers/VisualizerBase.hpp)
  — Basis-Klasse für Visualizer
- [../../src/visualizers/PulsingVisualizer.cpp](../../src/visualizers/PulsingVisualizer.cpp)
  — Referenz-Implementierung des Patterns
- [Qt-ADS-Dokumentation](https://github.com/githubuser0xFFFF/Qt-Advanced-Docking-System)
  — Docking-System
