...
**Kamera-Teardown-Deadlock: URSACHE GEFUNDEN UND BEHOBEN (S71, ⬜ Sichttest
am echten Gerät offen).** Der Wandler-Thread mappte ein `QVideoFrame` einer
gerade sterbenden MF-/D3D-Pipeline und blockierte im Treiber — dadurch gab er
den Pipeline-Puffer nie zurück und die nvwgf2umx-Worker konnten nicht sterben.
Log-Beweis: `wait(5000)` des Wandlers lief in jedem Lauf mit Feed in den
Timeout. Fix = **Abschalt-Vertrag** (Totflagge → Senke abklemmen → Barriere →
stop/zerstören, `LiveVideoFeed::feedStilllegen()`), Details in Changelog
1.60.0 und `include/services/LiveVideoFeed.md` §4a; die daraus abgeleitete
Regel steht als **Lebenszyklus-Vertrag** in
`docs/core-services/Bootstrap_Integration.md` §6.
