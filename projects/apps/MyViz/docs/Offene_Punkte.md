...
| **Sonde `convolution_kante`** | 4547/3981 px | 🟠 **S57 genau lokalisiert:** es ist die **letzte Zeile (y=239) und die letzte Spalte (x=319)**, jeweils vollständig — 320 + 240 = 560 Pixel, dazu ~400 verstreute. Dort steht in der Referenz **255**, bei uns **128**. **Randbehandlung ist als Ursache ausgeschlossen:** der Kern der Sonde ist `[0]*24 + [8] + [0]*24`, also nur die MITTE mit Gewicht 8 — es gibt gar keinen Nachbarzugriff. Der Unterschied muss aus der Skalierung/Klemmung kommen: `8 × 16 = 128` ist genau unser Wert, die Referenz sättigt dort auf 255. Nächster Schritt: `scale`/Klemmung bei Werten über 255 nachmessen (die acht Kennlinienpunkte aus S50 lagen alle im gültigen Bereich). `scale` geprüft, Kern seit S50 richtig orientiert | 🟠 |
