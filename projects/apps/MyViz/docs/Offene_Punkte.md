...
| **30 Bright Light District** | 0,252 → **0,070 / MAE 0,065** (320) · 0,206 → **0,019 / 0,018** (640, GRÜN) | 🟠 **S58: bei 640 grün, bei 320 auf EINEN Effekt eingegrenzt.** Gefallen sind hier der Dynamic-Shift-Umbau (dMean 0,270 → 0,026) und die `skip`-Semantik der Scopes (zwei der drei Flügel fehlten ganz). **Rest:** die gesamte Restabweichung entsteht an EINER Stelle — dem Movement an Position 3 (Stufe t02 misst 0,003, t03 dann 0,048; alle 32 Effekte danach addieren zusammen nur 0,009). Verbatim auf dem Referenzbild misst dieses Movement **MAE 0,144 in beiden Größen** bei gleichem dMean — die Streifen des `%3`-Musters sitzen also an anderer Stelle, nicht anders hell. **Die Zufallszahl ist ausgeschlossen:** mit festem `p` (drei Werte statt `rand(600)`) ändert sich nichts. Nächste Verdächtige im Skript: `pow(d+.001,-.35)`, die drei geschachtelten `if`-Ketten auf `d` und die Reihenfolge, in der `x` zweimal überschrieben wird | 🟠 |
