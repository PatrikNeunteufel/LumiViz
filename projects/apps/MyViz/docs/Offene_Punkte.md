...
Außerhalb der sechs Haken stehen noch zwei benannte §1-Treue-Reste
(**Tie Tunnel** `dt→dt_p` · **VR09-AVI-Vorschub**, beide erst durch die
S60-Werkzeuge messbar geworden) — Entscheid Patrik: gezielte Nacharbeit
(je eigene Session) oder §9-Abnahme.
...
3. 🟡 **Basis-Voreinstellungen für alle Module** (Strang G, Konzept §11) —
   ...
   **Erste Lieferung ✅ (S60, nach §0-Abschluss): 68 neue Vorlagen für 23
   ...
   Erklärungen in `asset/nodepresets/README.md`. **Offen:** Sichtprüfung
   Patrik im Betrieb (insb. die geschätzten Wertebereiche bei starfield/
   interferences/interleave) · weitere Typen (Import-Ernte aus
   `VisualsPresets` via Standalone) · host-eigene Module (Equalizer/Pulsing/
   Bloom/Fraktale — Hand-Design mit Sichtprüfung; Bloom-Werte ggf. aus
   `asset/effectchain/lights_demo.*` abgreifen).
